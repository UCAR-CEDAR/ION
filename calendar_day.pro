...
print, '<SELECT NAME=DAY SIZE=5>'

query='SELECT DISTINCT tbl_date.DAY FROM tbl_date,tbl_date_in_file WHERE tbl_date.DATE_ID=tbl_date_in_file.DATE_ID AND tbl_date.YEAR=' + string(year) + ' AND tbl_date.MONTH =' + string(month) + ' ORDER BY tbl_date.DAY ASC;'
rows=0ul
columns=0ul
results=0ul
cell=''
